...
def bootstrap_returns(
    ...
    block_size: int = 5,
    side: str = "greater",           #  "greater", "less", or "two-sided"
    seed: int = 0,
):
    """
    Moving-block bootstrap for the mean of *returns*.

    Returns
    -------
    orig_mean : float
    ci        : (low, high)
    p_value   : float          # one-sided or two-sided, see `side`
    """
    ...
    if n < 10:
        return r.mean(), (np.nan, np.nan), np.nan

    boot_means = np.empty(iterations)
    for i in range(iterations):
        resampled = []
        while len(resampled) < n:
            j = rng.integers(0, n - block_size + 1)
            resampled.extend(r[j : j + block_size])
        boot_means[i] = np.mean(resampled[:n])

    # percentile CI
    alpha = 1 - confidence
    ci_low, ci_high = np.percentile(boot_means, [alpha / 2 * 100, (1 - alpha / 2) * 100])

    # p-value under H0: mean <= 0  (or >=0, or =0 for two-sided)
    if side == "greater":            # H1: μ > 0
        p_val = np.mean(boot_means <= 0)
    elif side == "less":             # H1: μ < 0
        p_val = np.mean(boot_means >= 0)
    else:                            # two-sided
        p_val = np.mean(np.abs(boot_means) >= abs(r.mean()))

    return r.mean(), (ci_low, ci_high), p_val
...
def plot_bootstrap_analysis(
    ...
    plt.axvline(x=bootstrap_mean, color='red', linestyle='-', label=f'Mean: {bootstrap_mean:.6f}')
    plt.axvline(x=conf_interval[0], color='black', linestyle='--',
                label=f'95% CI: [{conf_interval[0]:.6f}, {conf_interval[1]:.6f}]')
    plt.axvline(x=conf_interval[1], color='black', linestyle='--')
    plt.axvline(x=0, color='gray', linestyle='--', label="Zero")
